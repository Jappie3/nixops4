...
pub(crate) async fn import_resource(
    ...
    eprintln!(
        "DEBUG resource: {:?}, with properties: {:?}",
    ...
    EvalSender::with(&eval_options.clone(), |s, mut r| async move {
        ...
        let r = {
            ...
            eprintln!(
                "DEBUG IMPORTING: {:?} {:?}, with properties: {:?}",
                resource_provider, resource_name, properties
            );
            ...
            let import_result = match imported_resource.as_ref() {
            ...
            // TODO this can't be RunState because the resource already exists
            // RunState calls Apply, we need Read
            ...
            //    Ok(other) => bail!("Unexpected outcome from ReadResource: {:?}", other),
            //    Err(e) => bail!("Failed to read resource: {}", e),
            //};

            //let state = state_handle.current.lock().await;
            //let r = serde_json::to_string_pretty(&*state)?;
            let r = serde_json::to_string_pretty(&import_result)?;
